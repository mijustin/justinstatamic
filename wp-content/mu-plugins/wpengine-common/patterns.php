...
$regex_is_user_agent_bot = 'MSIE 6|bot|facebook|crawler|indexer|feed|flipboard|slurp|re[ea]der|spider|scoutjet|ssowl|baidu|google|news|pubsub|^expo9|^tzogeoagent|jakarta|teoma|scrap(?:[ey]|ing)|siteimprove|nutch|coccoc|wazzup|genieo|thunderstone|Rackspace Monitoring|abonti|wpe-content-performance|bingpreview|SimplePie|yandex|duckduckgo|WordPress/[0-9]+\.[0-9]+.*verifying pingback';
...
$regex_is_user_agent_logged_as_bot = "bot|facebook|crawler|indexer|feed|flipboard|slurp|re[ea]der|spider|scoutjet|ssowl|baidu|google|news|pubsub|^expo9|^tzogeoagent|jakarta|teoma|scrap(?:[ey]|ing)|siteimprove|nutch|coccoc|wazzup|genieo|thunderstone|Rackspace Monitoring|abonti|wpe-content-performance|bingpreview|SimplePie|yandex|duckduckgo|WordPress/[0-9]+\.[0-9]+.*verifying pingback";
...
$regex_is_qargs_allowed_for_bot = "(?:p=\d+|feed=|amp([=&]|$))";

// Regex for whether a path should *not* be served to bots, in which case it 301s the bot to the home page
$regex_is_path_disallowed_for_bot = "(?!.*?/amp).*?(?:/page/\d\d+|/attachment/|/search/|/members/[^/]+/activity|/activity/p/)";
...
class Patterns
{
	...
	/**
	 * Checks a rewrite rule arg for any scary input
	 * @param string $str which is the argument tested for scariness
	 * @return bool if true, $str is equal to "$"
	*/
	static public function is_scary_regex_rule_arg($str){
